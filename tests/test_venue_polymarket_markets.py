...
class VenuePolymarketMarketsTest(unittest.TestCase):
    ...
    def test_parse_market_discovery_rows_keeps_only_expected_eligible_markets(self) -> None:
        ...
        self.assertEqual(btc_market.category, "crypto")
        self.assertEqual(btc_market.slug, "bitcoin-up-or-down-test")
        self.assertAlmostEqual(btc_market.volume_usdc, 1234.50)
        self.assertAlmostEqual(btc_market.liquidity_usdc, 321.25)
        self.assertAlmostEqual(btc_market.best_ask or 0.0, 0.42)
        self.assertAlmostEqual(btc_market.best_bid or 0.0, 0.40)
        self.assertAlmostEqual(btc_market.last_trade_price or 0.0, 0.41)
        self.assertEqual(btc_market.comment_count, 17)
        self.assertTrue(btc_market.neg_risk)
        self.assertEqual(btc_market.neg_risk_group_id, "neg-group-1")
        self.assertEqual(btc_market.total_duration_seconds, 850)
