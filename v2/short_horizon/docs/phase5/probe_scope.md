...
- **`micro_live_total_stake_cap_usdc`**: \$20.0
- **`max_daily_loss_usdc`**: \$10.0
- **`target_trade_size_usdc`**: \$1.0

*Note: These limits must be enforced by the configuration passed to the runtime.*
...
2. The bot does not exceed the predefined risk caps (no more than \$20 staked in total, no single trade over \$1, max loss hard-stopped at \$10).
