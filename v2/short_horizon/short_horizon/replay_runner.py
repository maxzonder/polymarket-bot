...
def replay_file(*, event_log_path: str | Path, db_path: str | Path, run_id: str | None = None, config: ShortHorizonConfig | None = None, config_hash: str = "dev") -> RunnerSummary:
    ...
    try:
        events = [event for event in ReplayEventSource(event_log_path).load() if _is_replay_input_event(event)]
...
def compare_bundle_to_replay(*, bundle_dir: str | Path, db_path: str | Path, replay_run_id: str | None = None):
    from .replay.comparator import compare_bundle_to_replay as _compare_bundle_to_replay

    return _compare_bundle_to_replay(
...
def render_comparison_report(report):
    from .replay.comparator import render_comparison_report as _render_comparison_report

    return _render_comparison_report(report)
