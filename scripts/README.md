...
строит таблицу `swans_v2` в `polymarket_dataset.db` с метриками entry/exit и метками победителей.
Запускается инкрементально (пропускает уже обработанные рынки).
...
и `ResolutionScorer` для знаменателей скоринга. Окно: Dec 2025 + Jan + Feb 2026.
...
Основной движок бэктеста. Реплеит события из `swans_v2` хронологически через
полный стек `OrderManager`. Поддерживает пессимистичную модель заполнения.
...
exposure v1.1, активность. С PR #48: `max_hours_to_close=720h` → universe ~8x шире,
ожидаемо видеть 80–200 resting bids вместо ~10.
