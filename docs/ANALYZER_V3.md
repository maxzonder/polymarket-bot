...
`analyzer_v2` хорошо находит **геометрические лебеди**:
- локальный минимум,
- затем значимый рост,
- `possible_x >= 5`.
...
`analyzer_v3` решает именно эту задачу:
...
Событие попадает в `token_swans_v3`, только если одновременно выполняются все условия:
...
```bash
cd ~/polymarket-bot
.venv/bin/python -m data_collector.analyzer_v3 \
  --date 2026-02-28 \
  --recompute \
...
`analyzer_v3` всё ещё не полноценный симулятор стакана.
...
`analyzer_v2` отвечает на вопрос:
- **где вообще были лебеди?**

`analyzer_v3` отвечает на вопрос:
