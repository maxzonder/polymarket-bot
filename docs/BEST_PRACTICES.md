...
- **Fee handling:** не полагаться на старую fee-модель в коде или заметках. После Fee Structure V2 (конец марта 2026) актуальная комиссия определяется через `feeSchedule` / `fee-rate`, а fee-enabled категорий стало больше, чем только crypto. Для ордеров лучше опираться на официальный client, который подставляет актуальный `feeRateBps`. Подробности: [TRADING_LOGIC.md](./TRADING_LOGIC.md) раздел 0.
